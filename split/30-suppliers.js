...
async function renderSuppliers() {
  const v = $("#view");
  const [suppliers, docs] = await Promise.all([all("suppliers"), all("docs")]);

  const pinvs = docs.filter(d => d.type === "PINV");
  ...
  const rows = suppliers
    ...
    .map(s => {
      ...
      return `
        <tr data-id="${s.id}">
          <td>${s.code || ""}</td>
          <td>${s.name || ""}${s.archived ? ' <span class="pill" style="margin-left:6px;background:#ddd;color:#333">archived</span>' : ""}</td>
  ...
  $("#sup_new").onclick = () => openSupplierForm();

  $$("#sup_rows [data-edit]").forEach(btn => {
    btn.onclick = () => openSupplierForm(btn.dataset.edit);
  });
}
