...
// Sales: Quotes / Orders / Invoices + Item Picker + Processing + Credit Notes
//
// New in this revision:
// - Batch Process (by date) all unprocessed invoices
// - Individual Process on an invoice form
// - Only processed invoices can be credited
// - Three sections: Active Invoices, Processed Invoices, Credit Notes (SCN)
// - Processed invoices become readOnly and cannot be changed
//
...
const __CREDIT_CFG = {
  CUSTOMER_INVOICE_TYPES: new Set(["INVOICE"]), // add "SINV" if you also use that
  CREDIT_DOC_TYPE: "SCN",
  MOVEMENT_TYPE_CREDIT: "SALE_RETURN", // movements for credit note (qtyDelta > 0)
  MOVEMENT_TYPE_SALE: "SALE",          // movements for invoice processing (qtyDelta < 0)
};

// ---------- Inline document overlay (print/email) ----------
(function () {
  ...
  window.showPdfOverlay = function showPdfOverlay(html, title, opts = {}) {
    ...
    overlay.querySelector("#doc_print").onclick = () => { try { iframe.contentWindow.focus(); iframe.contentWindow.print(); } catch {} };
    overlay.querySelector("#doc_newtab").onclick = () => { const w = window.open("", "_blank", "noopener"); if (w) { w.document.open(); w.document.write(html); w.document.close(); }};
...
(function attachCreditsAPI() {
  ...
  // Compute per-line remaining creditable qty (handles duplicates)
  ...
  async function createCreditNoteFromInvoice(invoiceId, creditQtyByLineId) {
    ...
    for (const ln of lines) {
      ...
      creditLines.push({
        ...
        qty: -Math.abs(qToCredit), // NEGATIVE line on SCN
    ...
    try {
      await add("adjustments", {
        ...
        docId: scn.id,
        relatedDocId: inv.id,
        amount: Math.abs(totals.grandTotal || 0), // positive reduces balance
        createdAt: nowISO(),
      });
    } catch {}
  ...
  window.openCreditNoteWizard = async function openCreditNoteWizard(invoiceId) {
    ...
    body.innerHTML = `
      ...
      <div class="bd">
        <div class="sub">Adjust quantities to credit (default = remaining).</div>
...
(async function attachProcessingAPI() {
    ...
    for (const ln of lines) {
      ...
      if (qty > 0) {
        await add("movements", {
          ...
          qtyDelta: -Math.abs(qty),         // issue stock out
          relatedDocId: doc.id,
          timestamp: nowISO(),
  ...
  async function processInvoice(doc, lines) {
    ...
    // Recompute totals to lock them in
  ...
  window.batchProcessInvoicesByDate = async function batchProcessInvoicesByDate(isoDate) {
    ...
    const target = (allDocs || []).filter(d =>
      d.type === "INVOICE" &&
      d.status !== "PROCESSED" &&
      (d.dates?.issue || "").slice(0,10) === isoDate
    );
    ...
    for (const d of target) {
      try {
        ...
        console.error("Batch process failed for", d.id, e);
      }
    }
...
// ===========================================================================
// SALES LISTS + FORMS (quotes/orders/invoices) with new sections & actions
// ===========================================================================

async function renderSales(kind = "INVOICE", opts = {}) {
  const invoiceView = opts.invoiceView || "active"; // 'active' | 'processed' (for INVOICE)
  const history = !!opts.history;                    // for QUOTE/ORDER
  ...
  // Split logic
  if (kind === "INVOICE") {
    if (invoiceView === "processed") {
      docs = docs.filter(d => d.status === "PROCESSED");
    } else { // active
      docs = docs.filter(d => d.status !== "PROCESSED");
    }
  } else if (kind === "QUOTE" || kind === "ORDER") {
    if (history) docs = docs.filter(d => d.status === "CONVERTED" || d.readOnly || d.convertedToId);
    else docs = docs.filter(d => !(d.status === "CONVERTED" || d.readOnly || d.convertedToId));
  }
...
// ---------- Sales Doc Form (Quotes / Orders / Invoices) ----------
async function openDocForm(kind, docId, opts = {}) {
  ...
  function wireAllRows() {
    if (readOnly) return;
    document.querySelectorAll("#sd_rows tr[data-idx]")?.forEach(wireRowEvents);
  }
  ...
  function openItemPicker(opts = {}) {
    ...
    wrap.tabIndex = -1;
    wrap.style.cssText = `position:fixed;inset:0;z-index:100000;background:rgba(2,6,23,.85);display:flex;align-items:center;justify-content:center`;
    wrap.addEventListener("keydown", (e) => e.stopPropagation(), { capture: true });
    wrap.innerHTML = `
      <div style="width:min(900px,94vw);max-height:80vh;background:#0f172a;color:#e2e8f0;border:1px solid #1f2937;border-radius:14px;box-shadow:0 20px 60px rgba(0,0,0,.55);display:flex;flex-direction:column;overflow:hidden">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:#0b1220;border-bottom:1px solid #1f2937">
          <b>Select Item</b><button type="button" class="btn" id="sd_picker_close">Close</button>
        </div>
        <div style="padding:12px 16px;display:grid;gap:10px;background:#0f172a">
          <input id="sd_picker_q" placeholder="Search code / name / barcode" style="min-width:320px;background:#0b1220;color:#e2e8f0;border:1px solid #1f2937;border-radius:8px;padding:8px 10px;">
          <div style="overflow:auto;max-height:52vh;border:1px solid #1f2937;border-radius:10px">
            <table class="table" style="width:100%">
              <thead><tr style="background:#0b1220"><th>SKU</th><th>Name</th><th class="r">On Hand</th><th class="r">Price</th><th></th></tr></thead>
    ...
    $("#sd_picker_close").onclick = () => wrap.remove();
    ...
    function wireList(list) {
      ...
      tbody.querySelectorAll("[data-pick]").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          if (it) { addLineFromItem(it, 1); wrap.remove(); }
        }, { capture: true });
      });
    }
  ...
  const draw = () => {
    ...
    if (!readOnly) {
      ...
      // Delete (blocked for INVOICE as policy)
